...
async function main() {
  ...
  const missingRef = "MOLLIE_SMOKE_MISSING_SECRET";
  ...
  try {
    ...
    const provider = await prisma.paymentProvider.create({
      data: { tenantId, type: "MOLLIE", mode: "TEST", status: "ACTIVE", credentialsRef: missingRef, config: { webhookTokens: [] } },
      select: { id: true },
    });
    ...
    const tx = await prisma.paymentTransaction.create({
      data: {
        ...
        orderDbId,
        providerId,
        externalId: "tr_smoke_missing",
        checkoutUrl: "https://mollie.local/checkout/tr_smoke_missing",
        status: "PENDING",
        amountMinor: 1234,
        currency: "UAH",
        currencyExponent: 2,
    ...
    await waitFor({
      label: "tx becomes PENDING_VERIFICATION + PROVIDER_AUTH_FAILED",
      timeoutMs: 30_000,
      stepMs: 250,
      fn: async () => {
        const row = await prisma.paymentTransaction.findUnique({
          where: { tenantId_id: { tenantId: tenantId!, id: transactionId! } },
          select: { status: true, resyncAttempt: true, nextResyncAt: true, lastErrorCode: true },
        });
        return (
          !!row &&
    ...
    console.log(JSON.stringify({ ok: true, tool: "paymentsLocalSmokeResyncTransactionMissingSecret", tenantId, providerId, transactionId }));
  } finally {
