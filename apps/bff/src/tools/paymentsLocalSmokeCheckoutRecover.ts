...
async function main() {
  ...
  const liqpaySecretRef = "LIQPAY_PRIVATE_KEY_REHEARSAL";
  ...
  try {
    process.env[liqpaySecretRef] = "priv_smoke";
    ...
    const provider = await prisma.paymentProvider.create({
      data: {
        ...
        config: {
          ...
          liqpay: {
            publicKey: "smoke_pub",
            currentSecretRef: liqpaySecretRef,
    ...
    await waitFor({
      label: "checkout.recover updates tx to PENDING with externalId + checkoutUrl",
      timeoutMs: 30_000,
      stepMs: 200,
      fn: async () => {
        const row = await prisma.paymentTransaction.findUnique({
          where: { tenantId_id: { tenantId: tenantId!, id: transactionId! } },
          select: { status: true, externalId: true, checkoutUrl: true },
        });
        return !!row && row.status === "PENDING" && row.externalId === transactionId && typeof row.checkoutUrl === "string" && row.checkoutUrl.length > 0;
      },
    });

    // eslint-disable-next-line no-console
    console.log(JSON.stringify({ ok: true, tool: "paymentsLocalSmokeCheckoutRecover", tenantId, providerId, transactionId }));
  } finally {
    if (process.env[liqpaySecretRef]) delete process.env[liqpaySecretRef];
