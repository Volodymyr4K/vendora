...
function hasMonobankPubkeys(config: unknown): boolean {
  if (!config || typeof config !== "object" || Array.isArray(config)) return false;
  ...
  return keys.some((k: any) => typeof k === "string" && k.includes("BEGIN PUBLIC KEY"));
}
...
async function main() {
  ...
  try {
    ...
    const rows = providers
      .map((p) => {
        ...
        return {
          ...
          hasPubkeys: hasMonobankPubkeys(p.config),
    ...
    const candidates = rows
      .filter((r) => r.status === "ACTIVE" && r.hasWebhookToken && r.hasPubkeys && r.hasCredentialsRef && r.credentialsEnvPresent)
      .slice(0, 20);
    ...
    console.log(
      JSON.stringify({
        ok: true,
        tool: "paymentsMonobankTenantDiscovery",
        mode,
        excludeSlug,
        candidates,
        summary: {
