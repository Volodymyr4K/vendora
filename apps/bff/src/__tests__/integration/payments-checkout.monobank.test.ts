...
describe("POST /payments/checkout (monobank)", () => {
  ...
  beforeAll(async () => {
    ...
    const prismaTx = {
      ...
      paymentTransaction: {
        ...
        findUnique: vi.fn().mockResolvedValue({ id: TX_ID, checkoutUrl: null, status: "INITIATED", providerId: "prov-mono", externalId: null }),
        create: vi.fn().mockResolvedValue({ id: TX_ID, checkoutUrl: null, status: "INITIATED", providerId: "prov-mono", externalId: null }),
    ...
    const prisma = {
      ...
      paymentTransaction: {
        findUnique: vi.fn().mockImplementation(async ({ select }: any) => {
          ...
          return {
            id: TX_ID,
            checkoutUrl: "https://pay.monobank.ua/page",
            status: "PENDING",
          };
        }),
    ...
    process.env.MONO_TOKEN = "mono-secret";

    fetchMock.mockResolvedValue({
      ok: true,
      status: 200,
      headers: new Headers({ "content-type": "application/json" }),
      text: async () => JSON.stringify({ invoiceId: "inv-1", pageUrl: "https://pay.monobank.ua/page" }),
    });
  ...
  it("200 when monobank invoice is created and persisted", async () => {
    ...
    expect(body.checkoutUrl).toBe("https://pay.monobank.ua/page");
