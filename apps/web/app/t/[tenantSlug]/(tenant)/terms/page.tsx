...
export default async function TenantTermsPage({ params }: { params: Promise<{ tenantSlug: string }> }) {
  ...
  const t = LEGAL_TEMPLATES.terms[locale];
  const legal = config.amContent?.legal?.terms;
                ...
                <div className="mt-10 border-t border-line/40 pt-10 space-y-10">
                  {t.sections.map((section) => (
                    <div key={section.title}>
                      <h2 className="text-2xl md:text-3xl font-serif text-ink mb-3">{section.title}</h2>
                      <p className="text-muted leading-relaxed text-[15px] md:text-[16px]">
                        {section.body}
                      </p>
                    </div>
