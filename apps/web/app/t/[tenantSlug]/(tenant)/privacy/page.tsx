...
export default async function TenantPrivacyPage({ params }: { params: Promise<{ tenantSlug: string }> }) {
  ...
  const t = LEGAL_TEMPLATES.privacy[locale];
  const cookieName = getAmLocaleCookieName(tenantSlug);
  const legal = config.amContent?.legal?.privacy;
          ...
          <div className="bg-bg border border-line shadow-sm p-8 md:p-12">
            {useOverride && overrideHtml ? (
              ...
              <>
                ...
                <div className="mt-10 border-t border-line/40 pt-10">
                  <h2 className="text-2xl md:text-3xl font-serif text-ink mb-4">{t.cookiesTitle}</h2>
                  <p className="text-muted leading-relaxed mb-6 text-[15px] md:text-[16px]">
                    {t.cookiesIntro}
                  </p>

                  <dl className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="border border-line bg-paper p-5">
                      <dt className="text-[10px] uppercase tracking-[0.25em] text-muted font-mono mb-2">Name</dt>
                      <dd className="font-mono text-sm text-ink break-all">{cookieName}</dd>
                    </div>
                    <div className="border border-line bg-paper p-5">
                      <dt className="text-[10px] uppercase tracking-[0.25em] text-muted font-mono mb-2">{t.cookiesPurpose}</dt>
                      <dd className="text-[15px] leading-relaxed text-ink/90">{t.cookiesPurposeValue}</dd>
                    </div>
                    <div className="border border-line bg-paper p-5">
                      <dt className="text-[10px] uppercase tracking-[0.25em] text-muted font-mono mb-2">{t.cookiesType}</dt>
                      <dd className="text-[15px] leading-relaxed text-ink/90">{t.cookiesTypeValue}</dd>
                    </div>
                    <div className="border border-line bg-paper p-5">
                      <dt className="text-[10px] uppercase tracking-[0.25em] text-muted font-mono mb-2">{t.cookiesLifetime}</dt>
                      <dd className="text-[15px] leading-relaxed text-ink/90">{t.cookiesLifetimeValue}</dd>
                    </div>
                  </dl>

                  <p className="mt-8 text-muted leading-relaxed text-[14px] md:text-[15px]">
                    {t.note}
                  </p>
                  <p className="mt-4 text-[10px] uppercase tracking-[0.25em] text-muted font-mono">
                    {t.updated}
                  </p>
                </div>
              </>
