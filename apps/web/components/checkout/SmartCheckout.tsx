...
export function SmartCheckout({ branchSlug, tenantSlug, initialAddresses, isAuthenticated, showTimeSlots = true }: SmartCheckoutProps) {
        ...
        const fetchSlots = async () => {
            ...
            try {
                ...
                if (!cancelled) {
                    ...
                    setSlots([]);
                    setTimeSlotsError("Order scheduling slots are temporarily unavailable. Please try again later.");
    ...
    if (stage === 'otp') {
        return (
            ...
            >
                <h3 id="otp-modal-title" className="text-xl font-bold mb-2">Confirmation</h3>
                <p className="text-muted mb-6">We sent a code to +380 {phone}</p>
        ...
        <div className="space-y-6">
            ...
            <Card className="p-4 rounded-theme">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <Button variant="ghost" onClick={() => cart.clear()} className="text-sm text-muted hover:text-danger">Clear</Button>
                </div>
                {!cart.items.length ? (
                    <div className="text-muted py-4 text-center">Cart is empty</div>
                ) : (
                        ...
                        {cart.items.map(it => (
                            <div key={it.id} className="flex justify-between items-start">
                                <div>
                                    ...
                                    <div className="text-xs text-muted">{formatPrice(it.priceSnapshot, true)} UAH</div>
                                </div>
            ...
            <Card className="p-4 rounded-theme space-y-4">
                ...
                {/* PERSON COUNT */}
                <div>
                    <Label className="block text-xs font-medium text-muted mb-1">Number of utensils</Label>
                            ...
                            {addresses.map(addr => (
                                ...
                                >
                                    ...
                                    <div className={`text-xs ${selectedAddressId === addr.id ? 'text-ink' : 'text-muted'}`}>
                                        {addr.city}, {addr.street} {addr.house} {addr.flat ? `, apt. ${addr.flat}` : ''}
                                    </div>
                                </div>
                            ))}
                            <Button
                    ...
                    {(showNewAddress || addresses.length === 0) && (
                        <div className="bg-[var(--bg)] p-4 rounded-theme border border-line animate-in fade-in space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                                <Input placeholder="City" value={newAddr.city} onChange={e => setNewAddr({ ...newAddr, city: e.target.value })} className="p-2 w-full" />
                                <Input placeholder="Street" value={newAddr.street} onChange={e => setNewAddr({ ...newAddr, street: e.target.value })} className="p-2 w-full" />
                            </div>
                            <div className="grid grid-cols-3 gap-3">
                                <Input placeholder="Bldg." value={newAddr.house} onChange={e => setNewAddr({ ...newAddr, house: e.target.value })} className="p-2 w-full" />
                                <Input placeholder="Apt./Office" value={newAddr.flat} onChange={e => setNewAddr({ ...newAddr, flat: e.target.value })} className="p-2 w-full" />
                                <Input placeholder="Label (Home)" value={newAddr.label} onChange={e => setNewAddr({ ...newAddr, label: e.target.value })} className="p-2 w-full" />
                            </div>
                ...
                {showTimeSlots && isScheduledOrderingEnabled && (
                    <div>
                        <Label className="block text-xs font-medium text-muted mb-1">Delivery time</Label>
                        ...
                        {deliveryType === 'scheduled' && (
                            <div className="animate-in slide-in-from-top-2 fade-in">
                                {loadingSlots ? (
                                    <div className="text-sm text-muted p-2 text-center bg-[var(--bg)] rounded-theme">Loading slots...</div>
                                ) : timeSlotsError ? (
                                    ...
                                    <div className="text-sm text-danger p-3 bg-danger-weak rounded-theme border border-danger">
                                        Unfortunately, no slots are available for today/tomorrow.
                                    </div>
                                ) : (
                                    <Select
                                        value={deliveryTime}
                                        ...
                                        options={[
                                            ...(!deliveryTime ? [{ value: "", label: "Select a time..." }] : []),
                                            ...slots.map(s => ({
                                                value: s.value,
                                                label: s.label,
                                                group: s.label.split(' ')[0] || "Other"
                                            }))
                                        ]}
                ...
                {/* COMMENT */}
                <div>
                    <Label className="block text-xs font-medium text-muted mb-1">Order comment</Label>
                    <Textarea
                        rows={2}
                        value={comment}
                        onChange={e => setComment(e.target.value)}
                        placeholder="Door code, courier instructions..."
                    />
                </div>
                ...
                {quote ? (
                    <div className="space-y-2 mb-4 text-sm">
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">{formatPrice(quote.subtotal, true)} UAH</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-muted">Delivery</span>
                            <span className="font-medium">{quote.deliveryFee === 0 ? "Free" : `${formatPrice(quote.deliveryFee, true)} UAH`}</span>
                        </div>
                        <div className="border-t pt-2 mt-2 flex justify-between text-lg font-bold">
                            <span>Total</span>
                            <span>{formatPrice(quote.total, true)} UAH</span>
                        </div>
                    </div>
                ) : (
                    <div className="text-center py-4 text-muted text-sm">Calculating total...</div>
                )}
